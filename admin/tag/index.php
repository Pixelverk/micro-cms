...
<form id="bulk-form" method="post" action="<?= e(url('admin/tag/bulk')) ?>" class="bulk-toolbar js-confirm-form" hidden
      data-confirm-title="<?= e(admin_trans('tag_delete_selected')) ?>"
      data-confirm="<?= e(admin_trans('tag_bulk_confirm')) ?>">
    <?= csrf_field() ?>

    <span class="bulk-count"><strong id="bulk-count">0</strong> <?= e(admin_trans('bulk_selected')) ?></span>

    <button type="submit" class="btn-small btn-danger"><?= e(admin_trans('tag_delete_selected')) ?></button>
...
<table class="content-table">
    ...
    <?php foreach ($tags as $tag): ?>
        <tr>
            <td class="col-select">
                <input type="checkbox" class="bulk-row" name="ids[]" value="<?= (int) $tag['id'] ?>"
                       form="bulk-form" aria-label="<?= e($tag['name']) ?>">
            </td>
            <td><?= e($tag['name']) ?></td>
