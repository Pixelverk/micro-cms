...
function docs_content(): array
{
            ...
            'sections' => [
                ...
                'Creating and editing content' => [
                    ...
                    ['ul' => [
                        ...
                        'Taxonomies: every taxonomy the theme offers this content type appears here, by name. One may allow several terms (Tags) and another only one (Category).',
                        'Parent: nest a page under another to build a URL like /services/consulting/.',
                        'Images: a Blog Post has a Featured image, and a Portfolio Item also has a Gallery. Pick from the media library, or paste a full URL.',
                    ]],
                ...
                'Bulk actions' => [
                    ['p' => 'Tick the boxes on the left of the content list to reveal a toolbar. You can publish, draft, archive, delete, clear cache, or add and remove a taxonomy term across the whole selection at once.'],
                    ...
                    ['p' => 'Taxonomies are managed the same way, with delete as the only bulk action: tick the terms on their list and remove them together. The content that used them is kept and simply loses the term.'],
    ...
    'layouts' => ['default' => 'Default', 'blog' => 'Blog Post'],\n    'headers' => ['site-header' => 'Default Header'],\n    'footers' => ['site-footer' => 'Default Footer'],\n    'defaults' => ['layout' => 'default', 'header' => 'site-header', 'footer' => 'site-footer'],\n    'menu_locations' => ['main' => 'Main Menu', 'footer' => 'Footer Menu'],\n    'content_types' => [\n        'page' => [\n            'label' => 'Page',\n            'default_layout' => 'default',\n            'available_components' => ['hero-section', 'cta-section'],\n            'url_prefix' => '',\n        ],\n        'blog_post' => [\n            'label' => 'Blog Post',\n            'available_components' => ['quill-editor'],\n            'editor' => 'rich-text',\n            'url_prefix' => 'blog',\n        ],\n    ],\n    'form_types' => [ /* contact, newsletter … */ ],\n    'styles' => ['utilities.css', 'style.css'],\n    'scripts' => [['src' => 'main.js', 'defer' => true]],\n    'icons' => ['favicon' => 'favicon.ico', 'app' => ['icon-192.png', 'icon-512.png']],\n];"],
                    ...
                    ['p' => "Taxonomies are declared at the top level under taxonomies. Core ships category and tag, so the key only needs the ones you add or override: url_prefix is the archive's first path segment, multiple says whether an item may carry several terms, and layout is the archive layout (taxonomy when absent)."],
                    ['code' => "'taxonomies' => [\n    'topic' => ['label' => 'Topic', 'url_prefix' => 'topic', 'multiple' => true, 'layout' => 'taxonomy'],\n],"],
                    ['p' => "A content type lists the taxonomies its editor offers under taxonomies; a type with none shows no taxonomy controls. Terms are shared, so every type that lists a taxonomy can pick any of its terms. Read an item's terms with content_taxonomies(\$page)[name], and link a term with taxonomy_url(name, slug)."],
                    ['p' => "defaults is the last fallback for a page's layout, header and footer: the page's own value wins, then the content type, then the settings, then this. Everything the manifest names — layouts, headers, footers, components, child names, styles, scripts, icons, taxonomies and form fields — is checked on the Health page, which reports anything that does not resolve."],
                ],
                'Writing a component' => [
                ...
                'Helpers available to themes' => [
                    ['table' => [
                        ...
                        'asset($path)' => 'Theme asset URL',
                        'theme_icon($name)' => 'Inline SVG icon from theme/assets/icons/',
                        'theme_icons()' => 'Every icon the theme ships, sorted',
                        ...
                        'render_image($value, $attrs)' => 'An image: a media id or a URL',
    ...
    'contact' => [
        ...
        'store_submission' => true,
                ...
                'The default theme as an example' => [
                    ...
                    ['p' => 'The demo also carries pages the reference has no counterpart for: /services/, /privacy/, the 404 page, and /landing/, which is the only page using the landing layout — a page with no header or footer. Between them and the search route, every layout the manifest declares renders on a fresh install, and the demo taxonomy terms are what make the blog archive and the generic taxonomy archive reachable: drop them and those two layouts have no page to appear on.'],
                ...
                'Menus' => [
                    ...
                    ['p' => 'get_menu_for_location() hands back items that are ready to render. Each is type, label, slug, target, hidden and children, plus two keys the resolver adds: url (empty when the link no longer resolves) and broken. type is a content type key, url for a hand-written link, or taxonomy for an archive (with the taxonomy named in its own key).'],
            ...
            'sections' => [
                ...
                'Roles' => [
                    ['table' => [
                        ...
                        'Editor' => 'All content including publishing and deletion, media, taxonomies, menus, forms.',
                ...
                'Maintenance' => [
                    ...
                    ['ul' => [
                        ...
                        'Export and system → Download Full Backup: downloads the whole site as a zip — the code, the database, the media library and the sitemap — which is how you move a site to another server.',
                        'Export and system → Run Migrations: applies schema updates after upgrading the code.',
                        'Content package → Export a package: opens a dialog with two downloads, content.json for the pages, posts, taxonomy terms and menus, and settings.json for the settings that describe them.',
                    ...
                    ['p' => 'A content package is JSON: content.json holds the pages, posts, portfolio items, taxonomy terms and menus, and settings.json holds the settings that describe the content — site title, homepage, layout defaults, URL prefixes and the menu assignments. Nothing carries a database id, so parents, taxonomy terms and the homepage are matched by slug when the package lands.'],
                    ...
                    ['h' => 'Moving the site to another server'],
                    ['p' => 'Unzip the backup into the web root of the new host: index.php, core/, admin/ and theme/ land where the site runs, and storage/ arrives with the database, the media library, the sitemap and the migration marker. Make storage/ writable by the web server user, then in config.php set url to the site\'s address (empty for a domain root) and env to production, and leave setup_completed true.'],
                    ['p' => 'The archive is written with security.form_secret removed, so the public form tokens fall back to a value derived from the install path. Set a new random secret in config.php if the site\'s forms are in use. BACKUP-README.txt inside the archive repeats these steps. The page cache, the logs, the sessions, the import stash and the test suite are deliberately not in the archive: the cache rebuilds itself, and the rest belongs to the old host.'],
                    ['p' => 'There is still no upload-and-restore button. A wrong database would brick the site, so restoring stays a step you take deliberately. To move content rather than an installation, use the content package above.'],
                ],
                'Upgrading' => [
