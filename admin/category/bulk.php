...
|--------------------------------------------------------------------------
|
| One POST endpoint for the categories list's bulk toolbar. Every id goes
...
if (!$ids) {
    redirect_with_toast('category', 'error', admin_trans('bulk_error_no_selection'));
}

if (count($ids) > TAXONOMY_BULK_LIMIT) {
    redirect_with_toast('category', 'error', admin_trans('bulk_error_limit', ['count' => TAXONOMY_BULK_LIMIT]));
}

$result = bulk_delete_taxonomies('category', $ids);

if ($result['removed'] === 0) {
    redirect_with_toast('category', 'error', admin_trans('bulk_error_none_changed'));
}

log_activity('taxonomy.bulk_delete', 'taxonomy', null, $result['removed'] . ' category(ies)', [
    'ids'     => $ids,
...
redirect_with_toast('category', $result['skipped'] > 0 ? 'error' : 'success', $message);
