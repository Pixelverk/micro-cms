...
* `theme/assets/icon-192.png` and `icon-512.png` are the placeholder app icons
  the web manifest falls back to (see `icons.app` in the manifest); keep them
  square PNGs. The theme's `favicon.ico` is a browser favicon only — an
