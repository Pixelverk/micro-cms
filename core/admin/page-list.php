...
<div class="page-header">
    ...
    <div class="page-actions">
        <a href="<?= url('admin/page-add') ?>" class="btn-primary">+ Add New Page</a>
    </div>
</div>
        ...
        <tbody>
            <?php foreach ($pages as $page): ?>
                <tr>
                    ...
                    <td class="actions">
                        <a href="<?= url('admin/page-edit') ?>?slug=<?= urlencode($page['slug']) ?>"
                           class="btn-small">
                            Edit
                        </a>
...
document.querySelectorAll('.delete-page-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (confirm(`Are you sure you want to delete the page "${slug}"?\n\nThis action cannot be undone.`)) {
            window.location.href = "<?= url('admin/page-remove') ?>?slug=" + encodeURIComponent(slug);
        }
    });
});
</script>
